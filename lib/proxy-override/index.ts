...
interface ProxyConfig {
  proxy: { baseUrlEnv: string; tokenEnv: string; hostPattern: string };
  families: Record<string, FamilyConfig>;
  localDetect: { baseUrlEnv: string; forceLocalEnv: string };
}
...
function _loadProxyConfig(): ProxyConfig {
  ...
  if (configRoot) candidates.push(path.join(configRoot, "config", "proxy.json"));
  ...
  for (const candidate of candidates) {
    try {
      if (fs.existsSync(candidate)) {
        const raw = JSON.parse(fs.readFileSync(candidate, "utf8")) as ProxyConfig;
        if (raw && raw.families) {
          _config = raw;
          return _config;
        }
...
function _family(config: ProxyConfig, provider: string | undefined, bareModel: string): string | null {
  const p = provider || "";
  for (const key of Object.keys(config.families)) {
    const m = config.families[key].match;
    if (m.provider && p === m.provider) return key;
    if (m.providerPrefix && p.indexOf(m.providerPrefix) === 0) return key;
    if (m.modelPrefix && bareModel.startsWith(m.modelPrefix)) return key;
  }
  return null;
}

function _prefixRe(config: ProxyConfig): RegExp {
...
export = { applyProxyOverride, _resetConfigCache, _loadProxyConfig };
