...
function _maxTokensField(modelId) {
...
function _convertMessages(messages, systemPrompt) {
    const result = [];
    if (systemPrompt)
    ...
    for (const msg of messages) {
        if (msg.role === "user") {
            ...
            if (Array.isArray(msg.content)) {
                ...
                if (reasoningParts)
                    m.reasoning_content = reasoningParts;
                result.push(m);
            }
            else {
...
class OpenAICompatAdapter extends IAIAdapter {
    ...
    async complete(req) {
        ...
        if (!baseUrl)
            throw new Error("openai-compat: missing baseUrl in policy binding");
        if (!apiKeyEnv)
            throw new Error("openai-compat: missing apiKeyEnv in policy binding");
        const apiKey = process.env[apiKeyEnv];
        if (!apiKey)
            throw new Error(`openai-compat: ${apiKeyEnv} not set`);
...
async function _consumeStream(resp, req) {
    let text = "";
    let reasoning = "";
    const toolCalls = {};
    ...
    if (finishReason === "tool_calls" || Object.keys(toolCalls).length > 0) {
        const content = [];
        if (reasoning)
            content.push({ type: "reasoning_content", text: reasoning });
        if (text)
