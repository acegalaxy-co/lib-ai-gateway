...
function _getClient(apiKeyEnv, baseUrl) {
...
class AnthropicAPIAdapter extends IAIAdapter {
    ...
    async complete(req) {
        ...
        const stream = await client.messages.create({ ...baseBody, stream: true }, { signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS) });
        const content = [];
        let current = null;
...
function _shapeNonStreaming(resp, req) {
...
module.exports = { AnthropicAPIAdapter };
