...
async function check(skill, tier) {
    ...
    if (binding.modelKey) {
        ...
        if (resolvedBaseUrl === undefined && row.baseUrl)
            resolvedBaseUrl = row.baseUrl;
        if (resolvedApiKeyEnv === undefined && row.apiKeyEnv)
            resolvedApiKeyEnv = row.apiKeyEnv;
    }
...
module.exports = { check, _reset, _modelsSource };
